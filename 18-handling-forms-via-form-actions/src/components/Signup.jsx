...
function signupAction(prevFormState, formData) {
    ...
    if (!terms) {
        errors.push('Please enter a valid terms');
    }

    if (acquisitionChannel.length === 0) {
        errors.push('Please enter a valid acquisition channel');
    }
...
export default function Signup() {
    ...
    // ? state management tied to form
    // ? state initial value also needs to be given
    const [formState, formAction] = useActionState(signupAction, {errors: null});

    return (
        /* ? when used in normal js projects, action specifies path to send request to, but in react it is converted to function execution */
